...
def upgrade() -> None:
    ...
    # Use conditional drops for fresh databases
    if conn.execute(sa.text("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'checkpoint_writes')")).scalar():
        op.drop_index(op.f('checkpoint_writes_thread_id_idx'), table_name='checkpoint_writes', if_exists=True)
        op.drop_table('checkpoint_writes')
    if conn.execute(sa.text("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'chat_history')")).scalar():
        op.drop_index(op.f('idx_chat_history_session_id'), table_name='chat_history', if_exists=True)
        op.drop_table('chat_history')
    if conn.execute(sa.text("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'checkpoints')")).scalar():
        op.drop_index(op.f('checkpoints_thread_id_idx'), table_name='checkpoints', if_exists=True)
        op.drop_table('checkpoints')
    if conn.execute(sa.text("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'checkpoint_blobs')")).scalar():
        op.drop_index(op.f('checkpoint_blobs_thread_id_idx'), table_name='checkpoint_blobs', if_exists=True)
        op.drop_table('checkpoint_blobs')
    if conn.execute(sa.text("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'feedback')")).scalar():
        op.drop_index(op.f('ix_feedback_id'), table_name='feedback', if_exists=True)
        op.drop_table('feedback')
    if conn.execute(sa.text("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'messages')")).scalar():
        op.drop_index(op.f('ix_messages_id'), table_name='messages', if_exists=True)
        op.drop_table('messages')
    if conn.execute(sa.text("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'checkpoint_migrations')")).scalar():
